...
@st.cache_data(ttl=300, show_spinner=False)
def get_stock_chart_data(ticker, yf_period, yf_interval):
    """يجلب بيانات السهم ويحسب المؤشرات الفنية للشارت التفاعلي"""
    df = pd.DataFrame()
    tv = get_tv()
    ...
    if tv is not None:
        try:
            ...
            tv_interval_val = tv_interval_map.get(yf_interval, Interval.in_daily)
            tv_ticker = ticker.replace('.CA', '')
            df_tv = tv.get_hist(symbol=tv_ticker, exchange='EGX', interval=tv_interval_val, n_bars=200)
            if df_tv is not None and not df_tv.empty:
                df_tv.rename(columns={'open':'Open', 'high':'High', 'low':'Low', 'close':'Close', 'volume':'Volume'}, inplace=True)
                if 'symbol' in df_tv.columns: df_tv.drop(columns=['symbol'], inplace=True)
                df = df_tv
    ...
    # Volume SMA
    df['Vol_SMA'] = df['Volume'].squeeze().rolling(window=10).mean()
